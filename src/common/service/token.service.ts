...
export class TokenService {
  ...
  detectSignatureLevel = async (role: RoleEnum = RoleEnum.user) => {
  ...
  decodedToken = async ({
    ...
    if (await this.tokenRepository.findOne({ filter: { jti: decoded.jti } })) {
      throw new UnauthorizedException('in-valid or Old Login ');
    }

    const user = await this.userRepository.findOne({ filter: { sub: decoded.sub } }) as UserDocument;
    if (!user) {
        ...
        data: [
          {
            ...
            expiredAt:new Date((decoded?.iat as number) +
